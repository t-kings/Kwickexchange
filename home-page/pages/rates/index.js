...
import rateStyle from "./Index.module.css";
import Footer from "../../components/footer";
import Nav from "../../components/navbar";
import { BitcoinContext, apiUrl, GiftCardContext } from "../../store/root";
import axios from "axios";
const Index = ({ bitcoinRate, giftCardRate }) => {
    ...
    return (
      <>
        ...
        <section className={style.content}>
          <div className={rateStyle.hold}>
            <div className={rateStyle.tabs}>
              <button
                onClick={() => this.setState({ ...this.state, formTab: 1 })}
                className={formTab === 1 ? rateStyle.active : ""}
              >
                Bitcoin
              </button>
              <button
            ...
            {formTab === 1 ? (
              bitcoinContext.sell.usd ? (
                ...
                >
                  ...
                  <input
                    className={rateStyle.link_btn_gold}
                    type="submit"
                    value="SELL"
                  />
                </form>
              ) : (
                <div className={rateStyle.load}>
                  <div className={rateStyle.loader}>Loading...</div>
                </div>
              )
                      ...
                      <div className={rateStyle.input}>
                        ...
                        >
                          <option value={0}>
                            ₦
                            {
                              giftCardContext.sell[giftCard].sellRates[country]
                                .cards[card].rate
                            }
                          </option>
                        </select>
...
export async function getServerSideProps() {
  ...
  return { props: { bitcoinRate, giftCardRate } };
}
